# Load packages
my_packages <- c("igraph", "readxl", "openxlsx", "tidyr", "dplyr", "plm","broom")
...
write.csv(kshap_w1_network_close_merged,"kshap_w1_network_close_merged.csv" )
...
# Get WEIGHTED degrees and export them to excel with nid
...
network_metrics_w1 <- data.frame(
  ...
  pagerank        = page_rank(g_w1, directed = TRUE, weights = E(g_w1)$weight)$vector,
  kcore_in        = coreness(g_w1, mode = "in"),
  kcore_out       = coreness(g_w1, mode = "out"),
  kcore_all       = coreness(g_w1, mode = "all")
)
hist(network_metrics_w1$degree_all)
...
network_metrics_w3 <- data.frame(
  ...
  # pagerank        = page_rank(g_w3, directed = TRUE, weights = E(g_w3)$weight)$vector,
  # kcore_in        = coreness(g_w3, mode = "in"),
  # kcore_out       = coreness(g_w3, mode = "out"),
  # kcore_all       = coreness(g_w3, mode = "all"),
  # wave            = 3
)
dim(network_metrics_w3)
dim(kshap_w3_network_close_merged)
dim(network_w3)
...
wave1 <- wave1 |>
  ...
  mutate(
    depressed = ifelse(cesd>=16, 1, 0),
    depressed = factor(depressed, levels = c(0,1), labels = c("non-depressed","depressed"))
)
...
wave3 <- wave3 %>%
  mutate(
    ...
    mmse_group = case_when(
      !is.na(mmse) & mmse <= 20 ~ "치매 의심",
      !is.na(mmse) & mmse > 20  ~ "정상",
      TRUE ~ NA_character_
    ),
    
    # 우울 여부
    depressed = factor(ifelse(cesd >= 16, 1, 0),
                       levels = c(0, 1),
...
vars_base <- c("mmse", "cesd", "age", "sex", "edu", 
               "income", "spouse_label", "degree_w_out", "self_health"
)
...
phtest(fe,re)
# 4) CRE 모델 (Mundlak approach)
library(plm)
library(dplyr)

# 1. 변수 선택
vars_cre <- c("cesd", "self_health", "spouse_label", "income_numeric",
              "age", "income", "degree_w_out")

# 2. 집단 평균(개인별 평균) 변수 추가
pdata_cre <- pdata %>%
  group_by(rdbid) %>%
  mutate(
